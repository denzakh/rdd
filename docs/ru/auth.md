...
- **TTL 12 часов** (смена в клинике), sliding renewal: при остатке < 6 часов
  сессия продлевается до полных 12 ч при валидации — но не дальше
  **абсолютного потолка 7 дней с момента логина** (`SESSION_ABSOLUTE_TTL_DAYS`
  в `session-repo.ts`): кража токена у активного пользователя даёт максимум
