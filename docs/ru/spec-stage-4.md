# Спека этапа 4: Качество, тесты и инфраструктура (docs/spec-stage-4.md)
...
2. **docs/matrix.md** — актуализировать: zustand и @tanstack/react-virtual уже в package.json (снять ⚠️),
   отметить реализованные разделы; после этапа 1 — дописать раздел про реальные мутации.
3. **docs/rdd-v1.md §8** — бэклог заменить ссылкой на `docs/roadmap.md`.
4. Добавить `docs/roadmap.md` в корень навигации docs (индекс спек этапов).
