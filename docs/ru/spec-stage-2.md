...
- Все агрегаты следуют инварианту де-идентификации (./export.md §3): уважают `data_scope` пользователя (тот же row-level access, что у списков) и подавляют ячейки с числом пациентов < k (по умолчанию `K_ANONYMITY_K = 5`).
...
- Опционально: `scripts/seed-demo.ts` — 5–10 тестовых пациентов с фазами для локальной разработки (только `--local`, на прод запрещён).
