...
- **Подгруппы секций (Вариант A):** Внутри секции поля могут иметь `group` (ссылка на ключ словаря подгрупп рядом с блоком реестра, например `THERAPY_GROUPS` в `src/shared/config/registry/therapy.ts`). При смене группы `buildMatrixRows` (`matrix-rows.ts`) вставляет subheader-строку (`kind: 'subgroup'`): фон `bg-muted/60`, `text-xs font-medium`, отступ `pl-6` — визуально тише заголовка секции. Инвариант «Поле = колонка» не затрагивается: группировка — только отображение (матрица + Data Dictionary), реестр остаётся плоским. Секция без словаря подгрупп (`SECTION_GROUPS` в `matrix-rows.ts`) рендерится плоским списком. Сейчас подгруппы есть только у секции «Фармакотерапия»: Депрессогенный фон, Соматическая поддержка, Антидепрессанты: классы, Курс АД: доза/путь/эффект, Нейролептики и транквилизаторы.
- **Скрытие строк грида (row-level):**
  - `hide_in_matrix` в реестре — поле не выводится строкой в гриде (авто-дубли: номер фазы виден в заголовке колонки, `hamd_severity`/`pure_remission`/`ad_any` — производные от видимых рядом значений). Поле остаётся в реестре, `applyComputed`, Data Dictionary и экспорте.
  - Deprecated-поля по версии протокола: строка исключается целиком, если ВСЕ фазы грида под версией ≥ `deprecated_since`; если хотя бы одна фаза старее — строка остаётся, а ячейки «withdrawn» фаз скрываются поштучно. Условия — `src/shared/lib/registry/field-availability.ts`, жизненный цикл — `./schema-evolution.md` §6.1.
...
- Строки-заголовки секций и подгрупп (`kind: 'section' | 'subgroup'`) не содержат ячеек — фокус «проскакивает» их к ближайшему полю в направлении движения (хелпер `nextFieldRow` в `matrix-grid.tsx`).
...
3. **Группировка батча по `phaseId` обязательна:** ячейки одной фазы → один Server Action `savePhaseCells` (`src/features/matrix/api/actions.ts`) с патчем (один UPDATE = атомарно). Разные фазы сохраняются независимо/параллельно. _(HTTP-роут `PATCH /api/phases/:id` в коде отсутствует — персист реализован через Server Actions.)_
4. ⚠️ **Флаш перед выгрузкой — не реализовано.** Проектировалось: `beforeunload` / `visibilitychange → hidden` — немедленная отправка pending-батча (`fetch` с `keepalive: true`), иначе правки за окно дебаунса теряются. В v1 не сделано: правки в окне дебаунса (≤300 мс) при закрытии/скрытии вкладки могут потеряться.
...
4. CAS включён всегда (страховка от двух вкладок/устройств одного пользователя). Формат ISO с `%f` лексикографически сортируем — позволит в будущем использовать `WHERE updated_at > ?` при polling (§6.3, не реализован).

### 6.3. Режимы работы (клиентская настройка) — спроектировано, не реализовано

> **Статус: в v1 не реализовано.** Активной синхронизации между пользователями в коде нет: чужие
> правки выявляются только в момент записи (CAS → 409, §6.4) или при перезагрузке страницы.
> Переключателя режима, polling и merge чужих правок в `src/widgets/matrix` и в маршруте матрицы
> нет; роут `GET /api/patients/:id/phases/changes` не существует. Ниже — проектное решение,
> кандидат на отдельный этап (см. `./roadmap.md`).

**Предполагаемое поведение (дизайн):**
...
5. После разрешения конфликта баннер снимается (в проектируемом режиме `collab`, §6.3, `since` для polling обновлялся бы на `serverUpdatedAt`).
...
### 6.7. Порог эскалации: polling → push-обновления — спроектировано, не реализовано

> **Статус: не реализовано** (см. §6.3). Активной синхронизации нет; раздел описывает проектное
> решение `collab` и порог, при котором рассматривается переход на push.

Проектируемый режим `collab` — polling каждые 30–60 с: осознанный компромисс v1 (без WebSocket/SSE-инфраструктуры, ноль дополнительных зависимостей, работает на Workers без Durable Objects). Обратная сторона — окно необнаруженного конфликта до интервала polling.
...
Когда polling будет реализован, он корректен: CAS гарантирует, что потеря данных невозможна в принципе, polling влияет только на скорость обнаружения, а не на целостность.
