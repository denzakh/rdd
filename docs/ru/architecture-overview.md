...
> сведены в [nfr.md](./nfr.md). Формализованная threat model (STRIDE) —
> [threat-model.md](./threat-model.md).
> Инфраструктура и эксплуатация (деплой на Cloudflare Workers, CI, домен, прод-База,
> откат и диагностика) разобраны в [deployment.md](./deployment.md).
...
Эволюция схемы — только через ресет БД (`npm run db:restart`): версионность протокола
(`registry_versions` + `registry_version` на записи) живёт в генерируемом baseline
`0001_init.sql`, отдельные `000N`-миграции под версионность не создаются
(см. [schema-evolution.md](./schema-evolution.md)).
...
фиксируется со значениями «до/после» и затёртой версией. Целостность закреплена на
уровне БД: триггеры запрещают UPDATE/DELETE, hash-chain (`prev_hash`/`entry_hash`,
миграция `0006_audit_hash_chain.sql`) детектирует ретроспективную подмену
(`verifyChain()`).
...
- **Регуляторика и комплаенс.** HIPAA / 152-ФЗ / GDPR-подобные требования, юридически
  значимые соглашения с пациентами, DPA с обработчиками — не реализованы и не
  имитируются. Система фиксирует только дату и версию формы информированного согласия
  при включении пациента в исследование ([consent.md](./consent.md)); текст согласия,
  электронная подпись и хранение сканов — вне scope. Демо-данные — синтетические,
  реальных персональных данных в системе нет и быть не должно.
...
- **Синхронизация коллаборации.** Активного обновления между пользователями нет: чужие
  правки выявляются только в момент записи (CAS → 409) или при перезагрузке страницы.
  Проектное решение — polling 30–60 с вместо WebSocket/SSE (осознанный компромисс v1);
  порог эскалации на push-обновления — при систематической параллельной работе врачей
  над одним пациентом (см. [matrix.md §6.3, §6.7](./matrix.md)).
