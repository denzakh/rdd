...
| 6   | The C4 diagram lives in `docs/diagrams/` — Next does not serve it                                                                           | Copy `docs/diagrams/c4-overview.svg` → `public/diagrams/c4-overview.svg`, render via `<Image unoptimized>`                                                                                                   |
...
- `src/widgets/landing/ui/landing.tsx` — sections:
  1. hero: title + subtitle from `README.md:1-5` + `PhD, Bekhterev Institute 2015` badge;
  2. CTAs: `Sign in` / `Docs on GitHub (architecture-overview.md)` / `Thesis abstract`;
