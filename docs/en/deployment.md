...
| `npm ci` fails inside `husky` / `prepare`                       | The install runs outside a git repository (husky needs git)                                | Run the install in a clone of the repository; to just install dependencies use `npm ci --ignore-scripts`                          |
...
npm ci                                              # 3. dependencies from the lockfile
npm run deploy                                      # 4. build + upload + Custom Domain creation
curl -I https://rdd.ux42.studio                     # 5. expect 200/302 (the certificate may take minutes)
npm run db:migrate:remote                           # 6. production D1 schema
npm run user:create:remote                          # 7. the first admin
npm run seed:demo:remote                            # 8. (optional) demo data
```
