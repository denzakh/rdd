...
- **Consent withdrawal stays a separate action** (`patients.consent_withdrawn_at`, button on the card): data is not deleted physically, the patient is excluded from reports/export (`entities/phase/api/queries.ts`, `entities/patient/api/patient-queries.ts`).
...
| Rule "withdrawn → out of reports"         | `src/entities/phase/api/queries.ts`, `src/entities/patient/api/patient-queries.ts`              |
...
| A legally significant primary source (signed scan/PDF) is required                       | scan in R2 (`MY_BUCKET` binding is added upon escalation, it is not in `wrangler.jsonc` yet) + authorized download, linked from the version register                                                                                                                   | a separate access-control contour; beyond the declared demo boundaries                                                     |
...
| Unsigned patients must be excluded from reports                                          | add `consent_version IS NOT NULL` to `CONSENT_OK` (`entities/patient/api/patient-queries.ts`) and to the phase queries; note that legacy records without a version will drop out of statistics                                                                         | a few SQL queries + tests                                                                                                  |
