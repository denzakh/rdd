...
1. **Виджет «Матрица» (`src/widgets/matrix`):** UI-компонент таблицы ввода и отображения динамических фаз заболевания, сгруппированный по секциям `REGISTRY` с условным рендерингом компонентов по полю `field.ui`. **Будет описана отдельным документом** (см. `docs/matrix.md`, когда будет создан).
2. **Слои `features` / `entities` / `app`:** наполнение недостающих слоёв FSD и подключение Server Actions.
3. **Подключить ZSA** (заявлен в стеке, пока не установлен).
