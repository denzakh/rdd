...
describe('consent: данные с отозванным согласием исключаются из отчётов', () => {
  it('countByField / phaseDurationsByOrder / efficacyByMainComponent', async () => {
    ...
    expect(countOf(await countByField(db, 'main_component'), 1)).toBeGreaterThan(0)
    expect((await phaseDurationsByOrder(db)).some((r) => r.phase_order_id === 1)).toBe(true)
    expect(
    ...
    expect(countOf(await countByField(db, 'main_component'), 1)).toBe(0)
    expect((await phaseDurationsByOrder(db)).some((r) => r.phase_order_id === 1)).toBe(false)
    expect(
      (await efficacyByMainComponent(db)).some((r) => r.main_component === 1 && r.ad_efficacy === 2)
    ).toBe(false)
