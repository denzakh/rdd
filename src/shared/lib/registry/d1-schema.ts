...
/**
 * Актуальная версия протокола CRF (docs/ru/schema-evolution.md §4).
 ...
 *  - consent_version — версия формы согласия (текст, напр. '1'); правится в
 *    форме регистрации/редактирования карточки;
 *  - consent_date — дата согласия (ISO-8601), проставляется автоматически при
 *    создании карточки;
 ...
 * Решение, что сознательно НЕ делаем (справочник версий, текст ИС, метка в
 * выгрузке), и пороги эскалации — docs/ru/consent.md.
 */
...
const toColumns = (table: TableId): TableColumn[] => {
  ...
  // Метка версии протокола (docs/ru/schema-evolution.md §4): системная колонка
...
/**
 * DDL журнала версий протокола (docs/ru/schema-evolution.md §4).
...
export const renderOpSql = (op: SchemaOp): string => {
  switch (op.kind) {
    case 'add':
      // registry_version — системная NOT NULL-колонка с DEFAULT (docs/ru/schema-evolution.md §4).
